...
namespace Foam
{
    defineTypeNameAndDebug(simpleHeatTransfer, 0);
...
Foam::incompressibleVars& Foam::simpleHeatTransfer::allocateVars()
{
...
void Foam::simpleHeatTransfer::addExtraSchemes()
{
...
void Foam::simpleHeatTransfer::continuityErrors()
{
...
void Foam::simpleHeatTransfer::solveTEqn()
{
    surfaceScalarField& phi = incoVars_.phiInst();
    const volVectorField& U = incoVars_.UInst();

    // Laminar (kinematic) viscosity from the transport model. For a
    // temperature-dependent / shear-thinning model this is the spatially
    // varying field computed from the current T and strain rate.
    const volScalarField nu(incoVars_.laminarTransport().nu());

    const volTensorField gradU(fvc::grad(U));

    // Kinematic viscous stress tau = nu (gradU + gradU^T)
    const volTensorField tau(nu*(gradU + gradU.T()));
...
Foam::simpleHeatTransfer::simpleHeatTransfer
(
    fvMesh& mesh,
...
:
    ...
    (
        ...
        (
            ...
            IOobject::MUST_READ,
            IOobject::NO_WRITE,
            false                       // do not register (incompressibleVars
                                        // creates its own registered copy)
        )
    ),
    T_
    (
    ...
    DT_("DT", dimensionSet(0, 2, -1, 0, 0, 0, 0), transportProperties_),
    cp_("cp", dimensionSet(0, 2, -2, -1, 0, 0, 0), transportProperties_),
...
bool Foam::simpleHeatTransfer::readDict(const dictionary& dict)
{
...
void Foam::simpleHeatTransfer::solveIter()
{
    preIter();
    mainIter();
    postIter();
...
void Foam::simpleHeatTransfer::preIter()
{
    Info<< "Time = " << mesh_.time().timeName() << "\n" << endl;
}


void Foam::simpleHeatTransfer::mainIter()
{
    // Grab references
    ...
    // Energy equation (uses the just-updated U); nu is then recomputed from
    // the new T (and strain rate) by laminarTransport().correct() below.
    solveTEqn();
...
void Foam::simpleHeatTransfer::postIter()
{
...
void Foam::simpleHeatTransfer::solve()
{
    // Iterate
    if (active_)
    {
...
bool Foam::simpleHeatTransfer::loop()
{
    return solverControl_().loop();
}


void Foam::simpleHeatTransfer::restoreInitValues()
{
    incoVars_.restoreInitValues();
}


void Foam::simpleHeatTransfer::preLoop()
{
...
void Foam::simpleHeatTransfer::postLoop()
{
    for (auto& obj : objectives_)
    {
...
bool Foam::simpleHeatTransfer::writeData(Ostream& os) const
{
