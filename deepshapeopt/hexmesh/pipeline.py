...
class SdfHexMeshPipeline:
    ...
    def sdf_at_phys(self, x_phys: "torch.Tensor") -> "torch.Tensor":
        """Differentiable design SDF in PHYSICAL distance units.

        ``phi > 0`` in the fluid, ``phi < 0`` inside the solid; carries the
        autograd graph to the design parameters (and to ``x_phys``). Used by
        the Transolver surrogate (query-cloud features, probe guards).
        """

        def _eval():
            sdf = self._make_sdf()
            return sdf.phi_ext(x_phys) / sdf.dist_scale
