...
1. the driver copies the experiment's ``dafoam_case/`` template (``0.orig``, ``constant``,
   ...
   driver :mod:`deepshapeopt.solvers.dafoam.runscript` is copied next to it;
...
Config block (``solver.dafoam``)::

    "solver": {"type": "dafoam", "case": "dafoam_case", "dafoam": {
...
from deepshapeopt.solvers.openfoam import runtime as foam_runtime

logger = logging.getLogger(__name__)

RUNSCRIPT = Path(__file__).with_name("runscript.py")
...
class DAFoamConfig:
    ...
    def from_dict(cls, cfg: dict) -> "DAFoamConfig":
        ...
        if unknown:
            raise ValueError(f"Unknown solver.dafoam keys {sorted(unknown)}; valid: {sorted(known)}")
        if "container" not in cfg:
            raise ValueError("solver.dafoam.container (path to the .sif image) is required")
...
def prepare_dafoam_case(template_dir: Path, run_name: str, runtime_root: Path | None = None) -> Path:
    ...
    case_dir = foam_runtime.prepare_foam_runtime(template_dir, run_name=run_name, runtime_root=runtime_root)
